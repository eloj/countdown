...
func main() {
	...
	flag.IntVar(&maxhits, "maxhits", 0, "Maximum number of results to return (0=unlimited)")
	...
	if cnt > 0 {
		fmt.Printf("Looking for solutions to '%s', minlen=%d, maxlen=%d, maxhits=%d, maxdist=%d\n", scramble, minlen, maxlen, maxhits, maxdist)
		start := time.Now()
		result := cw.FindWords(scramble, maxhits, maxdist)
